...
namespace DataBoss.Diagnostics
{
	...
	public class SqlServerMaintenancePlanWizard
	{
		...
		public bool RebuildStatistics = true;
		...
		public IEnumerable<SqlServerMaintenancePlan> CreateMaintenancePlans(SqlConnection server, string[] dbs)
		{
			...
			foreach (var database in dbs)
			{
				...
				if(RebuildStatistics)
					commands.Add("exec sp_updatestats");
				...
				foreach (var item in toProcess) {
					...
					commands.Add($"alter index [{item.IndexName}] on [{item.TableName}] {action}");
				}

				yield return new SqlServerMaintenancePlan(serverName, database, commands.ToArray());
				commands.Clear();
			}
		}
	}
