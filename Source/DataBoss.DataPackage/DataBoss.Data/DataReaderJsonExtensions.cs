...
namespace DataBoss.DataBoss.Data
{
	...
	{
		[JsonConverter(typeof(DataReaderJsonConverter))]
		[NewtonsoftJsonConverter(typeof(NewtonsoftDataReaderJsonConverter))]
		public class DataReaderJsonArray
		{
			...
			class NewtonsoftRecordWriter : IDisposable
			{
				...
				public Action<int> GetFieldWriter(Type t) =>
					Lambdas.CreateDelegate<Action<int>>(this,
						GetType().GetMethod(nameof(Serialize), BindingFlags.Instance | BindingFlags.NonPublic).MakeGenericMethod(t));

				void Serialize<T>(int ordinal) {
		...
		{
			public override bool CanConvert(Type objectType) => typeof(DataReaderJsonArray) == objectType;
			...
			public override void WriteJson(NewtonsoftJsonWriter writer, object value, NewtonsoftJsonSerializer serializer) =>
				((DataReaderJsonArray)value).WriteRecords(writer, serializer);
		}

		public class DataReaderJsonConverter : JsonConverter<DataReaderJsonArray>
		{
			public override DataReaderJsonArray Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options) {
				throw new NotImplementedException();
			}

			public override void Write(Utf8JsonWriter writer, DataReaderJsonArray value, JsonSerializerOptions options) =>
