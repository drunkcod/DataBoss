...
public static class MongoUtil
{
	...
	public static ObjectId? WriteJson(BsonReader bson, Utf8JsonWriter json) {
		...
		void WriteValue(int depth) {
			switch(bson.CurrentBsonType) { 
				...
				case BsonType.DateTime:  
					...
					json.WriteStringValue(v.ToString());
					break;
		...
		ObjectId? WriteDocument(int depth = 0) {  
			...
			while(bson.ReadBsonType() != BsonType.EndOfDocument) {
				...
				if(depth == 0 && p == "_id") {
					var v = bson.CurrentBsonType == BsonType.ObjectId ? bson.ReadObjectId() : bson.CurrentBsonType == BsonType.String ? ObjectId.Parse(bson.ReadString()) : throw new NotSupportedException($"Invalid _id {bson.CurrentBsonType}");
					WriteObjectId(v, json);
					id = v;
				} else {
					WriteValue(depth);
				}
	...
	public static ObjectId? RawWriteUtf8(RawBsonDocument b, Stream s) {
	...
	public static (int Changes, ObjectId[] Documents, BsonDocument? Resumetoken) GetChangedDocuments(IMongoCollection<BsonDocument> collection, ChangeStreamOptions options) {		
		var unset = new BsonDocument{ { "$unset", "fullDocument" } };
		...
		while(cur.MoveNext()) {
			...
			do { 
				++n;
				rows.Add(it.Current.DocumentKey.GetValue("_id").AsObjectId);
				resumeToken = it.Current.ResumeToken;
			} while(it.MoveNext());
		}

		return (n, rows.ToArray(), resumeToken);
	}
}
